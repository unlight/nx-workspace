...
export class AggregateRoot<TEvent extends Event = Event> {
  ...
  apply<T extends TEvent = TEvent>(event: T, isFromHistory = false) {
    if (isFromHistory) {
      this.getEventHandler(event)?.call(this, event);
      this.version++;
      return;
    }
  ...
  getUncommittedEvents() {
    return {
      ...
      events: [...this[INTERNAL_EVENTS]],
    };
  }

  protected getEventHandler<T extends TEvent = TEvent>(event: T): Function | undefined {
